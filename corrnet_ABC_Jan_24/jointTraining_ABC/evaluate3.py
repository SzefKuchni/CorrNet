...
def tf(view1,view2,labels,folder):
...
def tl3(folder, a,b):
    view1 = numpy.load(folder+"test-view"+a+".npy")
    view2 = numpy.load(folder+"test-view"+b+".npy")
    labels = numpy.load(folder+"test-labels.npy")
    tf(view1,view2,labels,folder)
...
def corr3(folder, a,b):
    print ("validation correlation")
    x = numpy.load(folder+"valid-view"+a+".npy")
    y = numpy.load(folder+"valid-view"+b+".npy")
    corr=compute_corr(x,y)
    detfile = open("res/corr_valid.txt", "a")
    detfile.write(folder)
    detfile.write(str(corr) + "\n")
    ...
    print ("test correlation")
    x = numpy.load(folder+"test-view"+a+".npy")
    y = numpy.load(folder+"test-view"+b+".npy")
    corr=compute_corr(x,y)
    detfile = open("res/corr_test.txt","a")
    detfile.write(folder)
    detfile.write(str(corr) + "\n")
...
#    print ("validation correlation of A to C among 2 nets")
#    x = numpy.load(folder1+"valid-view1.npy")
#    y = numpy.load(folder2+"valid-view2.npy")
#    compute_corr(x,y)
#
#    print ("test  correlation of A to C among 2 nets")
...
elif job=="tl3":  # example use of AB: python evaluate3.py tl3 TGT_DIR/ 1 2 
    tl3(sys.argv[2]+"project/",sys.argv[3],sys.argv[4])
elif job=="corr3":
    corr3(sys.argv[2]+"project/",sys.argv[3],sys.argv[4])
