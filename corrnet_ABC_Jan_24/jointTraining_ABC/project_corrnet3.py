...
plist = pickle.load(open(folder+"params.pck","rb"))
 #plist["n_visible_middle"],

model.init(l_rate=plist["l_rate"],
          tied=plist["tied"],
          n_visible_left=plist["n_visible_left"], n_visible_right=plist["n_visible_right"],n_visible_middle=392,#plist["n_visible_middle"],
          n_hidden=plist["n_hidden"], lamda=plist["lamda"], W_left=folder+"W_left",
          W_right=folder+"W_right",W_middle=folder+"W_middle", b=folder+"b", W_left_prime=folder+"W_left_prime",
          W_right_prime=folder+"W_right_prime",W_middle_prime=folder+"W_middle_prime", b_prime_left=folder+"b_prime_left",
          b_prime_right=folder+"b_prime_right",b_prime_middle=folder+"b_prime_middle", 
...
with tf.Session() as sess:
    ...
    new_mat = model.project_from_middle(mat)
    numpy.save(tgt_folder+"project/train-view2",new_mat.eval())
    
    mat = numpy.load(src_folder+"train/view3.npy")
    new_mat = model.project_from_right(mat)
    numpy.save(tgt_folder+"project/train-view3",new_mat.eval())
    ...
    mat = numpy.load(src_folder+"valid/view3.npy")
    new_mat = model.project_from_right(mat)
    numpy.save(tgt_folder+"project/valid-view3",new_mat.eval())
    ...
    mat = numpy.load(src_folder+"test/view3.npy")
    new_mat = model.project_from_right(mat)
    numpy.save(tgt_folder+"project/test-view3",new_mat.eval())
